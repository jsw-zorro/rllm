...
async def calculate_test_score(
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Test timeout for task '{task_name}' (rollout {rollout_id})")
            return 0.0
        ...
        try:
            ...
            logger.error(f"Parse error for {task_name}: {e}")
            return 0.0
        ...
        logger.error(f"Error in calculate_test_score: {e}", exc_info=True)
        return 0.0
...
def _calculate_weighted_test_score(
    ...
    if not test_weights:
        logger.warning(f"No test weights for task {task_name}, using equal weights")
    ...
    logger.warning(f"No matching test weights found for {task_name}, using equal weights")
    equal_weight = 1.0 / len(results)
    return sum(
        equal_weight if status == UnitTestStatus.PASSED else 0.0
        for status in results.values()
